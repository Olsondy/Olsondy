...
```text
请先阅读 AGENTS.md 以及与本任务相关的领域文档。
先检查当前实现并给出简短修改计划，再开始编辑。
只修改完成本任务所必需的文件，不重写无关代码。
完成后运行与改动匹配的检查命令，并报告验证结果。
如果发现规则、文档和代码不一致，请指出冲突，不要自行猜测。
```

这段提示词的重点不在措辞，而在顺序。它要求 Agent 先获得事实，再做决策；先产生可检查的计划，再产生代码。
...
如果你正在维护一个 vibe coding 项目，你可以直接下载这份 [AGENTS.md 项目治理模板](../../../../downloads/AGENTS-template.md)，复制到项目根目录后，再根据真实代码结构补充内容。

你可以这样给一组提示词给AI
```text
根据我的要求修改AGENTS.md, 我的要求是:尽量指出风格和方向性的内容以及绝对不能出现的规则
1. 所有标题大纲不能删除 
